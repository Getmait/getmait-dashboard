...
export default function MenuPage() {
  ...
  const [newItem, setNewItem] = useState({ name: '', nr: '', price: '', description: '', category: '' })
  ...
  // Kategorier udledt fra menudata
  const categories = useMemo(() => {
    const unique = Array.from(new Set(menuItems.map(i => i.kategori ?? 'Andet').filter(Boolean)))
    return ['Alle', ...unique.sort()]
  }, [menuItems])

  // Sæt default kategori til første kategori fra DB når menuen loades
  useEffect(() => {
    if (menuItems.length > 0 && newItem.category === '') {
      const firstCat = menuItems[0]?.kategori ?? 'Andet'
      setNewItem(prev => ({ ...prev, category: firstCat }))
    }
  }, [menuItems, newItem.category])
  ...
  const filteredMenu = useMemo(() =>
    menuItems.filter(item => {
      const matchesSearch =
        ...
        (item.nr ?? '').toLowerCase().includes(searchTerm.toLowerCase())
      const matchesCategory = activeCategory === 'Alle' || item.kategori === activeCategory
  ...
  async function addItem(e: React.FormEvent) {
    ...
    const { data } = await supabase
      ...
      .insert({
        ...
        nr: newItem.nr.trim() || null,
        kategori: newItem.category,
        beskrivelse: newItem.description.trim() || null,
    ...
    setNewItem({ name: '', nr: '', price: '', description: '', category: newItem.category })
    ...
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-[2.5rem] p-8 border border-slate-200 shadow-sm">
        {isAddingItem ? (
          ...
          <button
            ...
            className="w-full py-6 border-2 border-dashed border-slate-200 rounded-[2rem] flex items-center justify-center gap-4 text-slate-400 hover:border-[#ea580c]/40 hover:text-[#ea580c] transition-all group"
          >
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 text-slate-400 text-[9px] font-black uppercase tracking-[0.25em] italic border-b border-slate-100">
                ...
                <th className="px-10 py-6 text-right">Slet</th>
              </tr>
            </thead>
      ...
      <div className="bg-slate-900 rounded-[3rem] p-10 text-white flex flex-col md:flex-row items-center justify-between shadow-2xl relative overflow-hidden">
        <div className="relative z-10 flex items-center gap-8">
          <div className="bg-[#ea580c] p-5 rounded-[1.5rem] shadow-xl animate-pulse text-white">
